...
static ssize_t freg_proc_write(struct file* filp, const char __user *buff, unsigned long len, void* data){
	...
	if(len >PAGE_SIZE){
		printk(KERN_ALERT"[freg]The buff is too large: %lu.\n", len);
		return -EFAULT;
	}

	page = (char*)__get_free_page(GFP_KERNEL);
	if(!page){
		printk(KERN_ALERT"[freg]Failed to alloc page.\n");
		return -ENOMEM;
	}

	if(copy_from_user(page, buff, len)){
		printk(KERN_ALERT"[freg]Failed to copy buff from user.\n");
		err = -EFAULT;
		goto out;
	}
...
static int __init freg_init(void){
	...
	if(err < 0){
		printk(KERN_ALERT"[freg]Failed to alloc char dev region.\n");
		goto fail;
	}
	freg_major = MAJOR(dev);
	freg_minor = MINOR(dev);
	printk(KERN_ALERT"[freg]devno=(%d, %d)\n", freg_major, freg_minor);

	freg_dev = kmalloc(sizeof(struct fake_reg_dev), GFP_KERNEL);
	if(!freg_dev){
		err = -ENOMEM;
		printk(KERN_ALERT"[freg]Failed to alloc freg device.\n");
		goto unregister;
	}

	err = __freg_setup_dev(freg_dev);
	if(err){
		printk(KERN_ALERT"[freg]Failed to setup freg device: %d.\n", err);
		goto cleanup;
	}
	...
	if(IS_ERR(freg_class)){
		...
		printk(KERN_ALERT"[freg]Failed to create freg device class.\n");
		goto destroy_cdev;
	}
	...
	if(IS_ERR(temp)){
		err = PTR_ERR(temp);
		printk(KERN_ALERT"[freg]Faile to create freg device.\n");
		goto destroy_class;
	}

	err = device_create_file(temp, &dev_attr_val);
	if(err < 0){
		printk(KERN_ALERT"[freg]Failed to create attribute val of freg device.\n");
		goto destroy_device;
	}
	...
	printk(KERN_ALERT"[freg]Succeeded to initialize freg device.\n");
	return 0;
...
static void __exit freg_exit(void){
	...
	printk(KERN_ALERT"[freg]Destory freg device.\n");
